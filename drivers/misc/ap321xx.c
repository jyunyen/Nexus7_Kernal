 ...
 *
 * Summary:
 *	AP3212B /AP3212C/AP3216C device driver.
 *
...
#define AP321XX_DRV_NAME		"ap321xx"
...
#define	AL3212_PX_MSB_IR_MASK	0x6f
#define	AL3212_PX_LSB_IR_MASK	0x6f
...
#define AP3212B_PX_LTHH_SHIFT	(2)
...
#define AP3212B_PX_HTHH_SHIFT	(2)
#define AP3212B_PX_HTHH_MASK		0xff

#define AP3212B_PX_CONFIGURE	0x20

#define AP3212B_PX_ALGO 0x22
#define AP3212B_PX_ALGO_SHIFT	(0)
#define AP3212B_PX_ALGO_MASK	0x00
...
#define LDBG(s,args...) {}
#endif
static void lightsensor_work_handler(struct work_struct *w);
...
static struct ap321xx_data *private_pl_data = NULL;
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    ret = mod_timer(&data->pl_timer, jiffies + msecs_to_jiffies(200));
	...
	const char *buf, size_t count)
{
    ...
    //LDBG("DEBUG ap321xx function:line => %s : %d \n",__FUNCTION__,__LINE__);
    LDBG("DEBUG ap321xx_store_calibration_state..\n");
...
static int ap321xx_init_client(struct i2c_client *client)
{
    ...
    //	LDBG("DEBUG ap321xx function:line => %s : %d \n",__FUNCTION__,__LINE__);
    LDBG("DEBUG ap321xx_init_client..\n");
...
void pl_timer_callback(unsigned long pl_data)
{
    ...
    LDBG("%s\n",__func__);
    data = private_pl_data;//(struct ap321xx_data *)pl_data;
    queue_work(data->lightsensor_wq, &data->lightsensor_work);

    LDBG("Starting timer to fire in 200ms (%ld)\n", jiffies );
    ret = mod_timer(&private_pl_data->pl_timer, jiffies + msecs_to_jiffies(200));
...
static void lightsensor_work_handler(struct work_struct *w)
{
    ...
    struct ap321xx_data *data =
	container_of(w, struct ap321xx_data, lightsensor_work);
    u8 int_stat;
    int Pval;
	...
	const struct i2c_device_id *id)
{
    ...
    data->lightsensor_wq = create_singlethread_workqueue("lightsensor_wq");
    if (!data->lightsensor_wq) {
	LDBG("%s: create workqueue failed\n", __func__);
	err = -ENOMEM;
	goto err_create_wq_failed;
    }

    INIT_WORK(&data->lightsensor_work, lightsensor_work_handler);
...
err_create_wq_failed:
    ...
    if (data->lightsensor_wq)
	destroy_workqueue(data->lightsensor_wq);
exit_free_ps_device:
...
static int __init ap321xx_init(void)
{
    ...
    ret = i2c_add_driver(&ap321xx_driver);
...
MODULE_AUTHOR("John Huang Dyna-Image corporation.");
MODULE_DESCRIPTION("AP3212B, AP3212C and AP3216C driver.");
